...
def insert_batch_to_motherduck(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt > 0:
                logger.success(f"Successfully inserted batch {batch_num} on attempt {attempt + 1}")
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"All {max_retries} attempts failed for batch {batch_num}. Final error: {e}")
                raise
        finally:
            try:
                conn.unregister("cadent_batch")
...
def process_cadent_data(
    ...
    try:
        ...
        logger.success(f"Successfully read {total_rows:,} rows and {len(table.columns)} columns")
        ...
        for start in range(0, total_rows, batch_size):
            ...
            if success:
                ...
                logger.info(f"Inserted batch {batch_num}: {rows_inserted:,} / {total_rows:,} rows")
            else:
        ...
        logger.success(f"Completed! Inserted {rows_inserted:,} total rows into MotherDuck")
