...
WITH completed_emergency_works AS (
...
in_progress_emergency_works AS (
  SELECT DISTINCT ON (permit_reference_number)
  permit_reference_number,
  promoter_organisation,
  promoter_swa_code,
...
emergency_works AS (
  SELECT * FROM completed_emergency_works
  UNION ALL
  SELECT * FROM in_progress_emergency_works
)
SELECT * FROM emergency_works